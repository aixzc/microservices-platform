 ...
 * @returns 处理后的route数据
 */
export function rebuildRouteData(routes: any) {
    if (!routes) return;
    return routes.map((item: any) => {
        const data = buildThisRoute(item);
        if (item.subMenus && item.subMenus.length > 0) {
            data.children = rebuildRouteData(item.subMenus);
        }
        return data;
    });
}
...
/**
 * 组合本系统用的数据
 * @param data 后端返回的路由表数组
 * @returns 处理后的route数据
 */
export function buildThisRoute(data: any) {
    return {
        "path": data.url,
        "name": data.name,
        "component": data.path,
